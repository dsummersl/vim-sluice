function! sluice#renderer#RePaintMatches()"{{{
	...
	let current_dims = sluice#util#location#getwindowdimensions(
				\sluice#renderer#CombineData(g:mv_plugins,totalLines,firstVisible,lastVisible))
...
function! sluice#renderer#CombineData(plugins,totalLines,firstVisible,lastVisible)"{{{
	...
	for pluginInstance in a:plugins " now render everything
		...
		" TODO generally slow - this should be job_controll'd
